...
export function CollectiosnCard({ localPB,remotePB, collection }: CollectiosnCardProps) {
  ...
  return (
    <li
      ...
      className={
        checked
          ? "brightness-75 relative   w-[95%] flex flex-col justify-between bg-base-300 p-2 gap-2 rounded-lg group"
    ...
    >
      ...
      <div className="w-full flex flex-col justify-between gap-2 ">
        <PBTimeStamp timestamp={collection.created} label="Created"  className="w-full justify-start"/>
             <div className="w-full flex justify-between gap-2 ">
        <Suspense fallback={<div className="h-14 bg-base-100 skeleton" />}>
          <CollectionRecordsInfo pb={localPB} collectionName={collection.name} />
        </Suspense>
